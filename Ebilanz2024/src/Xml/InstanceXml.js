...
var InstanceXml = class InstanceXml {
    ...
    createInstance(param, contextList) {
        ...
        var xbrlIdentifier = xml.xml_createElement("xbrli:identifier", param['companyId'], attrsIdentifier);
        ...
        for (var j in param['schemaRefs']) {
            if (param['schemaRefs'][j].length > 0) {
                ...
                xbrlContent += '\n' + xml.xml_createElement("link:schemaRef", "", attrsSchemaRef);
            }
        }
        ...
        for (var i in contextList) {
            ...
            var xbrlInstant = '\n' + xml.xml_createElement("xbrli:instant", contextList[i].enddate) + '\n';
            var xbrlString = '\n' + xml.xml_createElement("xbrli:entity", xbrlIdentifier);
            xbrlString += '\n' + xml.xml_createElement("xbrli:period", xbrlInstant) + '\n';
            xbrlContent += '\n' + xml.xml_createElement("xbrli:context", xbrlString, attrsInstant);

            var attrsDuration = {
                id: "d-" + contextList[i].name
            };
            var xbrlDuration = '\n' + xml.xml_createElement("xbrli:startDate", contextList[i].startdate) + '\n';
            xbrlDuration += xml.xml_createElement("xbrli:endDate", contextList[i].enddate) + '\n';
            xbrlString = '\n' + xml.xml_createElement("xbrli:entity", xbrlIdentifier);
            xbrlString += '\n' + xml.xml_createElement("xbrli:period", xbrlDuration) + '\n';
            xbrlContent += '\n' + xml.xml_createElement("xbrli:context", xbrlString, attrsDuration);
        }
        ...
        var xbrlMeasure = '\n' + xml.xml_createElement("xbrli:measure", "iso4217:" + accountingBasicCurrency) + '\n';
        var xbrlUnit = '\n' + xml.xml_createElement("xbrli:unit", xbrlMeasure, { 'id': 'BaseCurrency' }) + '\n';
            ...
            for (var object in param.taxonomy[role]) {
                ...
                for (var i in contextList) {
                    ...
                    if (param.taxonomy[role][object][contextname] != 0) {
                        ...
                        xbrlContent += xml.xml_createElement(param.taxonomy[role][object]['qname'], Banana.SDecimal.round(param.taxonomy[role][object][contextname], { 'decimals': 2 }), attrs) + '\n';
                    }
                }
            }
        }
        ...
        results.push(xml.xml_createElement("xbrli:xbrl", xbrlContent, attrsNamespaces));
        return results.join('\n');
