...
var Xml = class Xml {
    ...
    xml_createElementWithValidation(name, content, mandatory, len, context, attributes) {
        ...
        if (fixedLen > 0 && content.length != fixedLen && xml.length > 0) {
            if (!content.length)
                content = '[vuoto]';
            msg = errors.getErrorMessage(ID_ERR_XML_LUNGHEZZA_NONVALIDA);
            ...
            if (!content.length)
                content = '[vuoto]';
            msg = errors.getErrorMessage(ID_ERR_XML_LUNGHEZZAMAX_NONVALIDA);
            msg = msg.replace("%1", name);
            msg = msg.replace("%2", content);
            ...
            if (!content.length)
                content = '[vuoto]';
            msg = errors.getErrorMessage(ID_ERR_XML_LUNGHEZZAMIN_NONVALIDA);
            msg = msg.replace("%1", name);
            msg = msg.replace("%2", content);
    ...
    xml_escapeString(_string) {
        ...
        var re = new RegExp(xml.AMPERSAND, 'g');
        string = string.replace(re, xml.ESCAPED_QUOTE[xml.AMPERSAND]);
        re = new RegExp(xml.GREATERTHAN, 'g');
        string = string.replace(re, xml.ESCAPED_QUOTE[xml.GREATERTHAN]);
        re = new RegExp(xml.LOWERTHAN, 'g');
        string = string.replace(re, xml.ESCAPED_QUOTE[xml.LOWERTHAN]);
        re = new RegExp(xml.APOS, 'g');
        string = string.replace(re, xml.ESCAPED_QUOTE[xml.APOS]);
        re = new RegExp(xml.QUOTE, 'g');
        string = string.replace(re, xml.ESCAPED_QUOTE[xml.QUOTE]);
        string = string.trim();
        return string;
    }
